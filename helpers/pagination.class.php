...
class Pagination {
    ...
    private $prev_string = "上一页";
    private $next_string = "下一页";
    ...
    private function initPage()
    {
        ...
        if(isset($this->param['page']) && $this->param['page']) {
            ...
            $this->page = isset($_GET['page']) ? $_GET['page'] : (isset($_POST['page']) ? $_POST['page'] : 1);
        }
        if($this->page < 1) {
            $this->page = 1;
        }
    ...
    private function escapePage()
    {
        ...
        $string[] = '<li class="page-item"><a class="page-link">总计：'.$this->total_count.'条记录</a></li>';
    ...
    {
        // 定义分页小项的字符串类型
        $unlink_string = "<li class='page-item disabled {class}'><a class='page-link'>{show}</a></li>";
        $link_string = "<li class='page-item {class}'><a class='page-link' data-page='{page}' href='{$this->url}{page}'>{show}</a></li>";

        if(count($array) == 2) {
            return str_replace(array('page-item {class}', '{show}'), $array, $unlink_string);
        }
        else {

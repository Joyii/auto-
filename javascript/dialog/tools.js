...
jQuery.dialog = function(title, container, options, _function) {
    ...
    jQuery.unscroll();
    if(utiler.formater(container, '[object string]') && (container.match(/^http/) || container.match(/^(\.\/|\.\.\/|\w)/))) {
        $.loading();
        $.ajax({ url: container, type: 'get', async: true, success: function(response) {
                ...
                $(id).find('.close').bind('click', function() {
                    ...
                    if(options.closeFunction) {
                        utiler.call(options.closeFunction);
                    }
                });
                utiler.call(_function);
                return true;
        ...
        if($(container).hasClass('already')) {
            $(container).parents('.dialog').eq(0).addClass('active');
        }
        else {
            if($(container).parents('.dialog').length) {
                $(container).parents('.dialog').append($(container));
            }
